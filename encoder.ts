...
export type AsyncWriter = WritableStreamDefaultWriter<Uint8Array>;

export async function writeFalse(writer: AsyncWriter) {
    await writer.write(new Uint8Array([0xF4]));
}
export async function writeTrue(writer: AsyncWriter) {
    await writer.write(new Uint8Array([0xF5]));
}
export async function writeNull(writer: AsyncWriter) {
    await writer.write(new Uint8Array([0xF6]));
}
export async function writeUndefined(writer: AsyncWriter) {
    await writer.write(new Uint8Array([0xF7]));
}
export async function writeHeader(writer: AsyncWriter, majorType: number, additionalInfo: number) {
    const header = new Uint8Array(1);
    header[0] = (majorType << 5) | additionalInfo;
    await writer.write(header);
}
export async function writeSimpleValue(writer: AsyncWriter, value: number) {
    if (value <= 31) {
...
export async function writeIntTiny(writer: AsyncWriter, number: number | bigint) {
    const { type, value } = getNumberWrittenValueAndType(number);
    await writeHeader(writer, type, Number(value));
}
export async function writeInt8(writer: AsyncWriter, number: number | bigint) {
    const { type, value } = getNumberWrittenValueAndType(number);
    await writeArgument8(writer, type, value);
}
export async function writeInt16(writer: AsyncWriter, number: number | bigint) {
    const { type, value } = getNumberWrittenValueAndType(number);
    await writeArgument16(writer, type, value);
}
export async function writeInt32(writer: AsyncWriter, number: number | bigint) {
    const { type, value } = getNumberWrittenValueAndType(number);
    await writeArgument32(writer, type, value);
}
export async function writeInt64(writer: AsyncWriter, number: number | bigint) {
    const { type, value } = getNumberWrittenValueAndType(number);
    await writeArgument64(writer, type, value);
}
export async function writeByteString(writer: AsyncWriter, value: Uint8Array) {
...
export async function writeTextString(writer: AsyncWriter, value: string) {
...
export async function writeInt(writer: AsyncWriter, number: number | bigint) {
    const { type, value } = getNumberWrittenValueAndType(number);
    await writeArgument(writer, type, value);
}

async function writeFloatN<ArrayConstructor extends typeof Float16Array | typeof Float32Array | typeof Float64Array>(writer: AsyncWriter, value: number | InstanceType<ArrayConstructor>, ArrayConstructor: ArrayConstructor, simpleValue: number) {
...
export async function writeFloat16(writer: AsyncWriter, value: number | Float16Array) {
    await writeFloatN(writer, value, Float16Array, AdditionalInfo.Length2);
}

export async function writeFloat32(writer: AsyncWriter, value: number | Float32Array) {
    await writeFloatN(writer, value, Float32Array, AdditionalInfo.Length4);
}

export async function writeFloat64(writer: AsyncWriter, value: number | Float64Array) {
    await writeFloatN(writer, value, Float64Array, AdditionalInfo.Length8);
}

export async function writeArrayHeader(writer: AsyncWriter, length?: number | bigint | undefined) {
    if (length === undefined) {
...
export async function writeMapHeader(writer: AsyncWriter, length?: number | bigint | undefined) {
    if (length === undefined) {
...
export async function writeValue(writer: AsyncWriter, value: PrimitiveWritableValue | WritableValue) {
