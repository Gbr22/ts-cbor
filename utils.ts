export type MapIterableToReturnType<T, R> =
    T extends Iterable<any> ? R :
    T extends AsyncIterable<any> ? Promise<R> :
    never;

export type MapIterableIteratorToReturnType<T, R> =
    T extends IterableIterator<any> ? R :
    T extends AsyncIterableIterator<any> ? Promise<R> :
    never;

export type AnyIterable<T = any> = Iterable<T> | AsyncIterable<T>;

export function collect<T, I extends AnyIterable<T> = AnyIterable<T>>(stream: I): MapIterableToReturnType<I, T[]> {
...
export function collectBytes<I extends AnyIterable<Uint8Array> = AnyIterable<Uint8Array>>(stream: I): MapIterableToReturnType<I, Uint8Array> {
    const result: MapIterableToReturnType<I, Uint8Array[]> = collect(stream);
    if (result instanceof Promise) {
        return result.then(chunks => concatBytes(...chunks)) as MapIterableToReturnType<I, Uint8Array>;
    }
    return concatBytes(...result as Uint8Array[]) as MapIterableToReturnType<I, Uint8Array>;
}
