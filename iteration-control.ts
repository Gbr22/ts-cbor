...
export class IterationControl<Yield, Exit> {
    ...
    >(
        iterate: (state: IterationState<Yield,PullValue,PullArgs>)=>Promise<void> | void,
        pullFn: SyncPullFn<PullValue,PullArgs> | AsyncPullFn<PullValue,PullArgs> = (()=>{}) as SyncPullFn<PullValue,PullArgs>
            ...
            while (true) {
                ...
                while (pullQueue.length > 0) {
                    ...
                    if (result instanceof Promise) {
                        throw new Error("Cannot use async pull function in sync iterator");
                    }
                    state.pulled.push(result);
                }
                try {
...
export function pullFunction<Fn extends (...args: any[])=>any>(...allArgs: PullFunctionArgs<Fn>): PullFunctionResult<Fn> | Promise<PullFunctionResult<Fn>> {
...
export type PullFunctionArgs<Fn extends (...args: any[])=>any> = [Fn, Parameters<Fn>, (value: Awaited<ReturnType<Fn>>)=>void];
export type PullFunctionIterationState<Yield, Fn extends (...args: any[])=>any> = IterationState<Yield, PullFunctionResult<Fn>, PullFunctionArgs<Fn>>;
